...
class KivanoBot:
    ...
    def show(self, args):
        if len(args) <= 0:
            ...
            if args == 'Фены' or args == 'Шины всесезонные' or args == 'Кофеварки' or args == 'Гладильные доски':
                fen = (f'Category {args}')
            else:
                fen = 'Not exist'


        if (args) not in self.kivanoset:
            return f'{args} - Not exist'
        else:
            advert_new = self.kivanos[self.kivanos.category_name == args]
